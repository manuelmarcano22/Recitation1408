...
scene.center = vec(1.5,.50,0) # center up and a little to the right of the origin
...
tabletop = box(pos=vec(0,1,0),size=vec(3,.2,1))
tableleg = cylinder(pos=vec(0,0,0),axis=vec(0,1,0),radius=.3) # big round leg in the middle 
table = compound([tabletop,tableleg])

# CREATE THE GROUND
ground = box(size=vec(4,.2,1),pos=vec(0,-.1,0),color=color.yellow)

# CREATE OUR BOOK TO SLIDE
book = box(pos=vec(0,1.15,0),size=vec(.3,.1,.2), color=color.green)
book.mass = .1        # .1 kg
...
weight = box(pos=vec(1.55,.5,0),size=vec(.1,.1,.1),color=color.green)
...
while not Finished:
    ...
    def change(env):
        weight.mass = weight.mass + 0.00001 # add clicked mass to total mass
    ...
    if book.moving: # if so, we need to move everything
        ...
        book.velocity = book.velocity + (book.acceleration * dt)
        book.pos = book.pos + (book.velocity * dt + .5 * book.acceleration * dt**2) 
        ...
        #string.pos = [ book.pos, vec(1.55,1.1,0), weight.pos ]
        string.visible =False
        string = curve(pos=[book.pos, (1.55,1.1,0)], color=color.red ) # string goes from book to slightly over edge
        string.append( [book.pos,(1.55,1.1,0),weight.pos] ) # string goes straight down
    ...
    masslabel = "\nTotal mass on string: {:.1f} {}.".format(weight.mass*1000, 'grams')
    labelmuk = "\nCoefficient of Kinetic Friction: {}".format(coefficientOfKineticFriction)
    labelmus = "\nCoefficient of Static Friction: {}".format(coefficientOfStaticFriction)
    #masslabel2 = s.format(weight.mass*1000, 'grams')
    #print(p) # "There are 13.5 liters."
    
    message = "String Tension: " + str(string.forceTensionMagnitude)
  #  messsage = tensionlabel
    #message += "\nMax Static Frictional Force: " + str(forceStaticFrictionOnBook)
    #message += "\nMax Kinetic Frictional Force: " + str(forceKineticFrictionOnBook)
    #message = "Total mass on string: {:.1f} {}.".format(weight.mass*1000, 'grams')
    message += masslabel
    message += labelmuk
    message += labelmus
